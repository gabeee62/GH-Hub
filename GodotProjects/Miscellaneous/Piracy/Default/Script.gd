...
@export var TextSettings: LabelSettings = LabelSettings.new() # The label's font, size, outline, etc. The only
# thing you shouldn't change in here is the font's color. It will apply the color change to every node that shares
# the set label settings, because you're changing the value of the actual LabelSettings file that they all reference.


@export var BackgroundVisible: bool = true # If you only want the button's text to appear instead of the full block, set this to false in the node's editor.
@export var BackgroundColor: Color = Color.DARK_GRAY # These color variables all have a default color, but because they are exports you can still change them. If you undo the change, they will go back to the specified default.
